...
const QString TrimmedValidatorFactory::isValid(const QModelIndex &index, QString &value) const {
    if (value.length() > 0) {
        if ((value.front().isSpace())) return formatMessage("%1 contains leading whitespace", index);
        if ((value.back().isSpace())) return formatMessage("%1 contains trailing whitespace", index);
    }
    return nullptr;
}
