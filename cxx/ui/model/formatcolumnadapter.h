...
template<class T>
class FormatColumnAdapter : public FieldColumnAdapter<T> {
...
public:
    ...
    FormatColumnAdapter(QString title, QVariant T::* field, FormatterType formatter, bool editable)
        : FieldColumnAdapter<T>(title, field, editable), formatter{formatter} {}

    virtual QVariant value(const T *row, const QModelIndex &index, const QVariant current, int role) const override {
        QVariant value = FieldColumnAdapter<T>::value(row, index, current, role);
        switch (role) {
        case Qt::DisplayRole:
