...
class EnumColumnAdapter : public FieldColumnAdapter<T> {
public:
...
public:
    EnumColumnAdapter(QString title, QVariant T::*field, QHash<const QString, const V*> *values,
        ...
        : FieldColumnAdapter<T>(title, field, editable, factory), values{values}, isCompatible{isCompatible} {}

    virtual QVariant value(const T *row, const QModelIndex &index, const QVariant current, int role) const override {
        auto value = FieldColumnAdapter<T>::value(row, index, current, role);
    ...
    virtual bool isEqual(const QVariant &value1, const QVariant &value2) const override {
        ...
        if (e1) return e2 && FieldColumnAdapter<T>::isEqual(e1->name, e2->name);
        return !e2;
    }

    virtual void setValue(T *row, QVariant value) const override {
        auto entity = value.value<const EnumValue*>();
        FieldColumnAdapter<T>::setValue(row, entity ? entity->code: QVariant{});
    }
};
